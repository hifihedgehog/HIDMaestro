...
Build the descriptor with [`HidDescriptorBuilder.AddPidFfbBlock`](sdk/HIDMaestro.Core/HidDescriptorBuilder.cs) — it emits the canonical "minimum viable" PID FFB block (one Feature report, Create New Effect 0x11). Do not add additional Feature reports (0x12, 0x13, 0x14) inside the same Application Collection; the four-feature variant from vJoy's reference descriptor causes pid.dll to AV inside `PID_EffectOperation+0x52` the first time the consumer calls `CreateEffect` on Windows 11 Build 26100 (issue #16). Pool, Block Load, and PID State are served from the separate shared-section path that doesn't touch pid.dll's preparsed-data parser. See [example/SdkDemo/Program.cs](example/SdkDemo/Program.cs) section 4b.
