...
- **Protocol controllers, not just passive HID.** The Nintendo Switch Pro Controller is not a passive device: hosts drive a Nintendo init-and-subcommand handshake and stall without a device that answers. HIDMaestro's driver answers it (0x80 handshake, SPI calibration reads, input-mode switch, 60 Hz report 0x30 streaming with gyro and accel), so SDL3's HIDAPI driver and Steam Input bind it as a real Pro Controller with motion and rumble. Before any protocol host arrives, the pad streams in the layout its HID descriptor declares, so joy.cpl and other DirectInput consumers read a working controller too, which is strictly better than real hardware's silent pre-handshake pad.
